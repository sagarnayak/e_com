...
impl AuthRolesCrossPaths {
    ...
    pub fn full_version(minified_version: String) -> Self {
        ...
        Self {
            ...
            readable_path : splits[4].to_owned(),
            get_allowed: if splits[5].to_owned() == "1" { true } else { false },
            post_allowed: if splits[6].to_owned() == "1" { true } else { false },
            put_allowed: if splits[7].to_owned() == "1" { true } else { false },
            delete_allowed: if splits[8].to_owned() == "1" { true } else { false },
            can_delegate_get: if splits[9].to_owned() == "1" { true } else { false },
            can_delegate_post: if splits[10].to_owned() == "1" { true } else { false },
            can_delegate_put: if splits[11].to_owned() == "1" { true } else { false },
            can_delegate_delete: if splits[12].to_owned() == "1" { true } else { false },
            can_access_for_children_get: if splits[13].to_owned() == "1" { true } else { false },
            can_access_for_children_post: if splits[14].to_owned() == "1" { true } else { false },
            can_access_for_children_put: if splits[15].to_owned() == "1" { true } else { false },
            can_access_for_children_delete: if splits[16].to_owned() == "1" { true } else { false },
            where_replacement: if splits[17].to_owned() != "0" { Some(splits[17].to_owned()) } else { None },
            created: None,
            modified: None,
        }
    }
